...
output_dir = os.path.expanduser("~/second_step_demo")
...
configuration["parameters"]["diversity_filter"] =  {
    "name": "IdenticalTopologicalScaffold",# other options are: "IdenticalTopologicalScaffold", 
                                           # "IdenticalMurckoScaffold" and "ScaffoldSimilarity"
                                           # -> use "NoFilter" to disable this feature
    "nbmax": 16,                           # the bin size; penalization will start once this is exceeded
    "minscore": 0.50,                      # 0.47 the minimum total score to be considered for binning
...
configuration["parameters"]["reinforcement_learning"] = {
    "prior": os.path.join(ipynb_path, "../../notebooks/models/random.prior.new"), # path to the pre-trained model
    "agent": os.path.join(ipynb_path, "./trained_agent/Agent.200.ckpt"), # path to the pre-trained model from the first step
...
scoring_function = {
    ...
    # add component: JOB
    {
        "component_type": "runjobs",            # run jobs for sampled molecules and parse log files 
        "name": "GFN2-xTB HOMO-LUMO gap",       # arbitrary name for the component
        "weight": 1,                            # the weight ("importance") of the component (default: 1) 
        "specific_parameters": {
            "transformation": {
                "transformation_type": "reverse_sigmoid",        # reverse sigmoid transformation 
                "low": 0,
                "high": 4,
                "k": 0.25
            },
            # The following parameters vary for different job management systems
            "project_name": "homo_lumo_gap_minimization",
            "tag": "demo",
            "chemconfig": "gfn2_xtb_opt_aizyn_batch",
            "job_name": "xtb_opt",
            "jobbuild_dir": "~/reinvent_jobs/build/",
            "jobparse_dir": "~/reinvent_jobs/parse/",
