...
class CoreReinforcementRunner(BaseRunningMode):
    ...
    def run(self):
        ...
        for step in range(self.config.n_steps):
            ...
            bucket_size = self._diversity_filter.parameters.bucket_size
            minscore = self._diversity_filter.parameters.minscore
            diversity_filter_memory = self._diversity_filter._diversity_filter_memory.get_memory()
            ...
            loss, agent_likelihood = self._inception_filter(self._agent, loss, agent_likelihood, prior_likelihood,
                                                            self.config.sigma, smiles, score, diversity_filter_memory, bucket_size, minscore)
            loss = loss.mean()
    ...
    def _inception_filter(self, agent, loss, agent_likelihood, prior_likelihood, sigma, smiles, score, diversity_filter_memory, bucket_size, minscore):
        self._inception.diversity_filter(diversity_filter_memory, bucket_size, minscore)
